...
export async function partitionPdfWithUnstructured(pdfPath: string): Promise<RawPartitionElement[]> {
  ...
  const scriptPath = resolve("server/slides/partition_pdf.py");
  try {
    const { stdout } = await execFileAsync(pythonExecutable, [scriptPath, pdfPath], {
      encoding: "utf8",
    ...
    const detail = error instanceof Error ? error.message : "Unknown partition error";
