...
def main() -> int:
    ...
    # Some Unstructured dependencies print warnings to stdout. Stdout is the
    # machine-readable channel consumed by Node, so route library output to stderr.
    with contextlib.redirect_stdout(sys.stderr):
        elements = partition_pdf(
            filename=str(pdf_path),
            strategy=os.environ.get("PDF_PARTITION_STRATEGY", "fast"),
            infer_table_structure=os.environ.get("PDF_INFER_TABLE_STRUCTURE", "false").lower() == "true",
        )
    payload = [
